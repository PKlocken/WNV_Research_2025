...
def model_DE_ShortTimeOnly_Simulation_NHost_Function(N, urban, transmit, infection_origin, num_years, savePath=None, extraInput=None, rtol=1e-3, atol=1e-6):
    
    # Create save path folder if not provided
    if not savePath:
    ...
    # Determine tag for urban parameter cases
    if 2 < urban < 5:
    ...
    # Short time span parameters
    T_short_initial = 0
    T_short_final = 250
    T_long_final = num_years * 365  # Keep for ODE final time
    ...
    # Compute disease-free equilibrium values
    Es_DFE = c_L * m_L * rs / (mu_V * m_E)
    Ls_DFE = c_L
    Vs_DFE = c_L * m_L / mu_V

    # Initial conditions for vector compartments based on infection origin
    ...
    # Calculate R0 and solve ODE with appropriate parameters
    if urban <= 2:
    ...
    # Extract host data short time
    hostData_short = x_short[:, 7:]
    hostData_short_final = x_short[-1, 7:]
    ...
    with open(txtFileName, 'w') as f:
        ...
        f.write("Final Host Compartment Values (Short Time):\n")
        for j, val in enumerate(hostData_short_final, start=1):
            f.write(f"  x({j}): {val:.6f}\n")
         # Final densities summed over each group of three (like MATLAB)
        f.write("Final Host Densities (Short Time Simulation):\n")
        for j in range(N):
            start_idx = 3 * j
            end_idx = start_idx + 3
            host_sum = hostData_short_final[start_idx:end_idx].sum()
            f.write(f"  Host {j+1}: {host_sum:.6f}\n")

    return hostData_short_final, Max_Prevalence, R0, MH_ratio, MH_ratio_a, v_R0
